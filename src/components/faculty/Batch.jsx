...
function App()
{
	...
	let {user,faculty,student,courses,slots,batch,faculty_name,student_name,slots_name,course_name,date}=state
	...
	const p3=(e,name) =>{
		...
		remarks:"add attendance all"
		}

		batch=[...batch,ob]
		dispatch({type:"batch",payload:batch})
		dispatch({type:"message",payload:"batch created success"})
	}
	...
	return <div className="wrapper batch">
		<div className="b-heading">
			<div className="chip">{course_name}</div>
			<div className="chip">{faculty_name}</div>
			<div className="chip">time would be : {slots_name} </div>
		</div>
